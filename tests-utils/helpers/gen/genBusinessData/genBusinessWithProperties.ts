...
interface TestProperty {
  ...
  images?: string[]; // Paths to local test images
...
const defaultTestProperty: TestProperty = {
...
export const genBusinessWithProperties = async (
  businessName?: string,
  properties: Partial<TestProperty>[] = [{}]
) => {
  let userId: string;
  let email: string;
  ...
  const propertyIds = [];  const hostelsCollection = db.collection("hostels");
  ...
  return {
    ...
    email,
    business_name,
    propertyIds, // Return the IDs of created properties
  };
};
